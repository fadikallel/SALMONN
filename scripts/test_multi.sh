...
# if (( $ARNOLD_ID % 2 == 1 )); then
#     # ARNOLD_ID 为奇数，减 1
#     EFFECTIVE_ID=$((ARNOLD_ID - 1))
#     RANK=1
# else
#     # ARNOLD_ID 为偶数，保持不变
#     EFFECTIVE_ID=$ARNOLD_ID
#     RANK=0
# fi

EFFECTIVE_ID=$(( (ARNOLD_ID / GROUP_SIZE) * GROUP_SIZE ))
RANK=$((ARNOLD_ID % GROUP_SIZE))

# 构造 HOST_ADDR
HOST_ADDR=METIS_WORKER_${EFFECTIVE_ID}_HOST

# ${ARNOLD_WORKER_GPU}
torchrun --nproc_per_node=${ARNOLD_WORKER_GPU} --nnodes=$GROUP_SIZE --node_rank=$RANK --master_addr="${!HOST_ADDR}" --master_port=$((12800 + EFFECTIVE_ID)) \
