...
def compute_metrics_from_batched_outputs(data, positive_label='spoof'):
    ...
    for batch in data:
        ...
        raw_predictions = batch.get('text', [])
        predictions = []
        for p in raw_predictions:
            p = str(p).strip().lower()
            if p.startswith("bonafide"):
                predictions.append("bonafide")
            elif p.startswith("spoof"):
                predictions.append("spoof")
            else:
                predictions.append("unknown")  # or whatever fallback you prefer    
...
def compute_detailed_metrics_by_category(data, category_key='task', positive_label='spoof'):
    ...
    for batch in data:
        categories = ['bonafide', 'spoof']  # Example categories, replace with actual category values
        ground_truth = batch.get('ground_truth', [])
        raw_predictions = batch.get('text', [])
...
if __name__ == "__main__":
    ...
    with open('outputs/202607011209/eval_test_epoch_0.json', 'r') as f:
        data = json.load(f)
